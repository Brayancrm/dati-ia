...
const SETORES_COLUNAS = [
  ...
  { nome: 'TELEFONES', valor: 'telefones', icone: <svg className="w-6 h-6 text-green-700" fill="none" stroke="currentColor" strokeWidth={2} viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" d="M3 5h2l.4 2M7 13h10l4-8H5.4M7 13l-1.35 2.7A2 2 0 007.48 19h9.04a2 2 0 001.83-1.3L17 13M7 13l1.5-6h7l1.5 6" /></svg> },
...
const GRUPOS_DADOS = [
  ...
  },
  {
    nome: 'TELEFONES',
    icone: (
      <svg className="w-6 h-6 text-green-700" fill="none" stroke="currentColor" strokeWidth={2} viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" d="M3 5h2l.4 2M7 13h10l4-8H5.4M7 13l-1.35 2.7A2 2 0 007.48 19h9.04a2 2 0 001.83-1.3L17 13M7 13l1.5-6h7l1.5 6" /></svg>
    ),
...
export default function DashboardPage() {
  ...
  const executarBusca = async ({ cpfBusca, tipoBusca }: { cpfBusca: string, tipoBusca: string }) => {
    ...
    try {
      ...
      // Descontar crédito apenas se não for master
      const isMaster = user.email && user.email.toLowerCase() === "brayan@agilisvertex.com.br";
      if (!isMaster) {
        const usuarioDoc = (await getDocs(query(collection(db, "usuarios"), where("email", "==", user.email)))).docs[0];
        if (usuarioDoc) {
  ...
  const executarBuscaLote = async ({ chaves }: { chaves: string[] }) => {
    ...
    let chavesValidas = 0; // Contador de chaves válidas para desconto de créditos
    ...
    for (let i = 0; i < chaves.length; i++) {
      ...
      try {
        ...
        if (!tipoBusca || valorBusca.length === 0) {
          ...
          continue; // Não incrementa chavesValidas para formatos inválidos
        }
        
        chavesValidas++; // Incrementa apenas para chaves com formato válido
    ...
    // Descontar créditos apenas para chaves válidas
    const isMaster = user.email && user.email.toLowerCase() === "brayan@agilisvertex.com.br";
    if (!isMaster && chavesValidas > 0) {
      const usuarioDoc = (await getDocs(query(collection(db, "usuarios"), where("email", "==", user.email)))).docs[0];
      if (usuarioDoc) {
        const usuario = usuarioDoc.data();
        await updateDoc(doc(db, "usuarios", usuarioDoc.id), { creditos: usuario.creditos - chavesValidas });
        setUsuarioInfo({ ...usuario, creditos: usuario.creditos - chavesValidas });
      }
    }
    setLoteLoading(false);
  ...
  return (
    <div className="w-full px-1 sm:px-2 md:px-4 bg-white py-2 mt-2">
      ...
      <section className="bg-white rounded-xl shadow-lg p-6 border-2 border-yellow-400/60">
        ...
        {!modoLote && (
          <div className="mb-4">
            <h3 className="font-semibold text-gray-900 mb-2">Busca Individual:</h3>
            <form className="flex flex-col sm:flex-row gap-4 items-center" onSubmit={handleBusca}>
              <input type="text" placeholder="Digite o CPF ou CNPJ" className="border-2 border-gray-200 rounded-lg px-3 py-2 flex-1 focus:outline-none focus:border-yellow-400 transition text-black placeholder:text-gray-700" value={cpfBusca} onChange={e => handleInputBusca(e.target.value)} />
              <select className="border-2 border-gray-200 rounded-lg px-3 py-2 focus:outline-none focus:border-yellow-400 transition text-black" value={tipoBusca} onChange={e => setTipoBusca(e.target.value)}>
                <option className="text-black" value="CPF">Confirmação de identidade (CPF)</option>
                <option className="text-black" value="CNPJ">Ciclo de crédito (CNPJ)</option>
              </select>
              <button type="submit" className="p-2 bg-green-900 text-yellow-400 rounded-lg hover:bg-yellow-400 hover:text-green-900 transition shadow-md border-2 border-green-900" title="Buscar" disabled={buscando}>
                {buscando ? <span className="animate-pulse">Buscando...</span> : <svg width="20" height="20" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><circle cx="11" cy="11" r="8"/><line x1="21" y1="21" x2="16.65" y2="16.65"/></svg>}
              </button>
            </form>
          </div>
        )}
        {buscaMsg && <div className="mt-4 text-red-700 font-semibold">{buscaMsg}</div>}
        {loteMsg && <div className="mt-4 text-red-700 font-semibold">{loteMsg}</div>}
        ...
        {resultado && (
          <div className="mt-6 bg-gray-50 border border-yellow-400 rounded-lg p-4">
            {/* Cabeçalho principal */}
            <div className="bg-green-50 border border-yellow-400 rounded-xl p-3 mb-3 flex flex-col md:flex-row md:items-center md:justify-between gap-3 shadow">
              <div className="flex-1">
                <div className="flex items-center gap-3 mb-2">
                  <svg className="w-8 h-8 text-green-900" fill="none" stroke="currentColor" strokeWidth={2} viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" d="M5.121 17.804A13.937 13.937 0 0112 16c2.5 0 4.847.655 6.879 1.804M15 11a3 3 0 11-6 0 3 3 0 016 0z" /></svg>
                  <span className="text-xl font-extrabold text-green-900 tracking-tight">{resultado.NOME || resultado.NOME_COMPLETO || 'Nome não informado'}</span>
                </div>
                <div className="flex flex-wrap gap-4 text-sm text-gray-800 mb-2">
                  {resultado.CPF && <span><b>CPF:</b> {resultado.CPF}</span>}
                  {resultado.RG && <span><b>RG:</b> {resultado.RG}</span>}
                  {resultado.SEXO && <span><b>Gênero:</b> {resultado.SEXO}</span>}
                  {resultado.DATA_NASCIMENTO && <span><b>Nascimento:</b> {resultado.DATA_NASCIMENTO}</span>}
                  {resultado.IDADE && <span><b>Idade:</b> {resultado.IDADE}</span>}
                  {resultado.SIGNO && <span><b>Signo:</b> {resultado.SIGNO}</span>}
                </div>
                <div className="flex flex-wrap gap-4 text-sm text-gray-800">
                  {resultado.NOME_MAE && <span><b>Mãe:</b> {resultado.NOME_MAE}</span>}
                  {resultado.NOME_PAI && <span><b>Pai:</b> {resultado.NOME_PAI}</span>}
                </div>
              </div>
              <div className="flex flex-col gap-2 items-end">
                <span className="inline-flex items-center px-3 py-1 rounded-full bg-green-200 text-green-900 font-bold text-xs">
                  {resultado.STATUS || 'ATIVO'}
                </span>
                {resultado.PERSONA && (
                  <span className="inline-flex items-center px-3 py-1 rounded-full bg-yellow-200 text-yellow-900 font-bold text-xs">
                    <svg className="w-4 h-4 mr-1" fill="none" stroke="currentColor" strokeWidth={2} viewBox="0 0 24 24"><circle cx="12" cy="12" r="10" /><circle cx="12" cy="10" r="3" /><path d="M12 13v2" /></svg>
                    {resultado.PERSONA}
                  </span>
                )}
              </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-3">
              {setorizarDados(resultado).map(setor => (
                <div key={setor.nome} className="bg-white rounded-xl shadow p-2 border border-gray-200">
                  <div className="flex items-center gap-2 mb-2">
                    {setor.icone}
                    <span className="font-bold text-sm text-gray-800">{setor.nome}</span>
                  </div>
                  {setor.dados.length > 0 ? (
                    <div className="space-y-1">
                      {setor.dados.map(d => (
                        <div key={d.label} className="flex flex-col">
                          <span className="text-[10px] font-semibold uppercase tracking-wide text-gray-700 mb-0.5">{d.label}</span>
                          <span className="text-xs font-mono break-all text-gray-900 flex items-center">
                    ...
                    <div className="text-gray-400 flex flex-col items-center py-4">
                      <svg className="w-8 h-8 mb-1" fill="none" stroke="currentColor" strokeWidth={2} viewBox="0 0 24 24"><rect x="3" y="3" width="18" height="18" rx="2" /><path d="M9 9h6v6H9z" /></svg>
                      <span className="text-xs">Nenhum dado encontrado.</span>
                    </div>
                  )}
                </div>
              ))}
